...
| items | either Core schema meta-schema or schemaArray; left out means yes | no |  |
...
| if | Core schema meta-schema | no |  |
| then | Core schema meta-schema | no |  |
| else | Core schema meta-schema | no |  |
...
- **Core schema meta-schema** (used 14)
- **schemaArray** (used 4)
